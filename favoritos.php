...
<body>
    ...
    if (empty($_SESSION['id'])) {
        echo "<h3>Para agregar productos a tu lista de favoritos debes iniciar sesión en tu cuenta de <strong>PlayONRetro.</strong></h3>"; 
        ?> 
        <br> 
    ...
    $consulta = "SELECT * FROM favoritos WHERE usuario_id = '$usuario_id' AND producto_id = '$producto_id'";
    $verificacion = mysqli_query($conexion, $consulta);

    if (mysqli_num_rows($verificacion) <= 0) {
        $agregar = "INSERT INTO favoritos (usuario_id, producto_id) VALUES ('$usuario_id', '$producto_id')";
        mysqli_query($conexion, $agregar);
    } else {
        $eliminar = "DELETE FROM favoritos WHERE usuario_id = '$usuario_id' AND producto_id = '$producto_id'";
